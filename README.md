...
Currently, SEEDS is distributed as pre-compiled binaries:
    ...
    Download the version for your OS from the Releases section.
    ...
    Access the Dashboard: The program will display a local URL (e.g., http://192.168.1.15:44069). Simply type this address into the browser of any other device on the same Wi-Fi.
...
When running SEEDS, you might encounter Windows SmartScreen or Defender warnings:
    ...
    How to bypass: 1. Click "More info".
    2. Click "Run anyway".
    3. If the Firewall prompt appears, select "Allow access" for Private Networks, otherwise other devices won't be able to find your PC.

🚀 How to Use

    Host the Server: Start the app on your main PC.

    Connect: Copy the displayed URL or use the QR code (if available) to open the interface on your phone or another laptop.

    Upload: Use the "Upload" button to send files to the host PC. They will appear in the ./downloads folder.

    Share: Place any files you want to distribute into the ./shared folder next to the app. They will instantly appear for everyone connected.
